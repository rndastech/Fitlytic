...
const detectPose =
    async (webcamRef, canvasRef, onResultCallback, drawSkeleton) => {
  if (!poseLandmarker) {
    ...
    if (webcamRef.current && webcamRef.current.video.readyState >= 2) {
      try {
        poseLandmarker.detectForVideo(
            webcamRef.current.video, performance.now(), (result) => {
              ...
              canvasCtx.drawImage(
                  webcamRef.current.video, 0, 0, canvas.width, canvas.height);
              ...
              for (let i = 0; i < sortedLandmarks.length; i++) {
                ...
                if (pose) {
                  const nonFaceLandmarks = result.landmarks[0].filter(
                      (_, index) => index > 10 && (index < 17 || index > 22));
                  drawingUtils.drawLandmarks(
                      nonFaceLandmarks,
                      {color: i === 0 ? 'red' : 'purple', radius: 2.5});
